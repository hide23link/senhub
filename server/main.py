"""
Senhub テスト用サーバー（メモリストレージのみ・DB実装なし）
...
設定変更: server/.env を編集（.env.example を参考に）

注意: プロセス終了時にデータは消える（DB実装は後で追加予定）
"""
...
def _get_channel(channel_id: int) -> dict:
    """チャンネルを取得（存在しなければ自動作成）"""
